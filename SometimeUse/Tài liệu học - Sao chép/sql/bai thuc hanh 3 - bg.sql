...
-- Viết truy vấn thực hiện các yêu cầu sau:
-- 1. Hiển thị mã số lớp, tên lớp, tên khoa của tất cả các lớp
select MaLop, TenLop, TenKhoa
from Lop, Khoa
where Lop.MaKhoa = Khoa.MaKhoa

-- 2. Hiển thị thông tin chi tiết của các lớp ở khoa "Công nghệ thông tin"
select *
from Lop, Khoa
where Lop.MaKhoa = Khoa.MaKhoa and TenKhoa= 'Cong nghe thong tin';
-- 3. Hiển thị họ tên các sinh viên ở lớp có tên "K46CB"
select * from Lop, Khoa
where Lop.MaKhoa = Khoa.MaKhoa and TenLop= 'K46CB'
-- 4. Đưa ra họ tên các sinh viên ở khoa có mã số "K01"
select HoTen
from SinhVien, Lop
...
where a.MaLop = b.MaLop and MaKhoa = 'K01'

-- 5. HIển thị họ tên, tên lớp, tên khoa của tất cả các sinh viên
select HoTen, TenLop, TenKhoa
from SinhVien a, Lop b, Khoa c
where a.MaLop =b.MaLop and b.MaKhoa= c.MaKhoa

-- 6. HIển thị họ tên, ngày sinh của tất cả các sinh viên ở khoa "Công nghệ thông tin"
select HoTen, NgaySinh
from SinhVien a, Lop b, Khoa c
where	a.MaLop= b.MaLop
	and b.MaKhoa= c.MaKhoa
	and TenKhoa='Cong nghe thong tin'
-- 7. Hiển thị họ tên, ngày sinh của tất cả các sinh viên ở khoa "Công nghệ thông tin" và khoa "Điện tử viễn thông"
select HoTen, NgaySinh, TenKhoa
from SinhVien a, Lop b, Khoa c
where	a.MaLop= b.MaLop
	and b.MaKhoa= c.MaKhoa
	and (TenKhoa='Cong nghe thong tin' 
			or TenKhoa='Dien tu vien thong')
-- 8. Hiển thị mã số các khoa có địa chỉ ở "Đống Đa"
select MaKhoa
from Khoa_DiaDiem, DiaDiem
where Khoa_DiaDiem.MaDD= DiaDiem.MaDD
		and DiaDiem.DiaChi='Dong Da'

select MaKhoa
from Khoa_DiaDiem a, DiaDiem b
where a.MaDD= b.MaDD
		and b.DiaChi='Dong Da'
-- 9. Hiển thị tên các khoa có địa chỉ ở Nguyễn Huệ
select TenKhoa
from Khoa_DiaDiem a, DiaDiem b, Khoa c
where	a.MaDD = b.MaDD
	and	a.MaKhoa = c.MaKhoa
	and b.DiaChi = 'Nguyen Hue'
-- 10. Hiển thị tên, địa chỉ của khoa có mã số là "K02"
select TenDD, DiaChi
from Khoa_DiaDiem a, DiaDiem b
where a.MaDD = b.MaDD
and a.MaKhoa = 'K02'
-- 11. HIển thị tên địa điểm của khoa "Toan"
select TenDD
from Khoa_DiaDiem a, DiaDiem b, Khoa c
where	a.MaDD = b.MaDD
	and	a.MaKhoa = c.MaKhoa and TenKhoa='Toan'
